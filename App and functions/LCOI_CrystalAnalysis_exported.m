classdef LCOI_CrystalAnalysis_exported < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        SorthexagonalButton             matlab.ui.control.Button
        ...
        PlotdeviationsButton_3          matlab.ui.control.Button
        ...
        PlotdeviationsButton_2          matlab.ui.control.Button
    ...
    methods (Access = public)
        ...
        function  plotDirectionColor(app,ax,typeP)
            ...
            if typeP==1 % Hexagonal
                if isempty(app.coorOfMasksHexa)==false
                    ...
                    if v=='2'
                        ...
                        figure(50)
                        clf;
                        plotColorWheel(app,struct('symmetry', 2))
                    else
                        % Plotting the directions of the unit-cells colorcoded
                        ...
                        figure(50)
                        clf;
                        plotColorWheel(app,struct('symmetry', 6))
                    end
                end
            elseif typeP==2 % Rectangular
                ...
                elseif v=='4'
                    ...
                    figure(50)
                    clf;
                    plotColorWheel(app,struct('symmetry', 4))
                end
        ...
        function plotColorWheel(~,optionsCol)
            % Creates a color wheel with customizable symmetry
            %
            % Parameters:
            %   options - Structure with fields:
            %     .symmetry  - Symmetry fold (2, 3, 4, 6, etc.) [default: 2]
            %     .minAngle  - Starting angle in degrees [default: 0]
            %     .maxAngle  - Ending angle in degrees [default: 180]

            % Set defaults
            if nargin < 1
                optionsCol = struct();
            end
            if ~isfield(optionsCol, 'symmetry')
                optionsCol.symmetry = 2;
            end
            if ~isfield(optionsCol, 'minAngle')
                optionsCol.minAngle = 0;
            end
            if ~isfield(optionsCol, 'maxAngle')
                optionsCol.maxAngle = 360/optionsCol.symmetry;
            end
            ...
            % Generate grid for the wheel
            [x, y] = meshgrid(linspace(-rmax*1.1, rmax*1.1, resolution));
            [theta, rho] = cart2pol(x, y);

            % Create the HSV color space
            hueImage = zeros(size(theta));
            saturationImage = ones(size(theta));
            valueImage = ones(size(theta));

            % Apply wheel mask
            wheelMask = (rho >= rmin) & (rho <= rmax);
            saturationImage(~wheelMask) = 0;

            % Check if using full circle or exact segment
            segmentSize = 360 / optionsCol.symmetry;
            maxShifted=optionsCol.maxAngle;
            if optionsCol.minAngle>0
                maxShifted=optionsCol.maxAngle-optionsCol.minAngle;
            end
            if maxShifted>segmentSize
                maxShifted=segmentSize;
                errordlg('The chosen range is too wide for the symmetry. A narrower range is displayed','Range Error!');
            end

            % Map angles to hue based on symmetry
            for i = 1:numel(theta)
                if wheelMask(i)
                    ang = mod(mod(theta(i)-optionsCol.minAngle*pi/180,2*pi), (2*pi/optionsCol.symmetry));
                    % Check if this point is in the valid range
                    inRange =  ang*180/pi <= maxShifted;

                    if inRange
                        % In range - map to hue [0,1]
                        hueImage(i) = ang / (maxShifted*pi/180);
                    else
                        % Out of range - set to red (hue = 0)
                        hueImage(i) = 0;
                    end
                end
            end
    ...
    methods (Access = private)
        ...
        function LoadProjectButtonPushed(app, event)
            ...
            % Plot some of the results of the loaded project
            UpdatelayerssButtonPushed(app, [])
            if isempty(app.coorOfMasksHexa)==false && isempty(app.coorOfMasksRect)==true && isempty(app.coorOfMasksTri)==true
                SorthexagonalButtonPushed(app, [])
                if isempty(app.directions)
                    PlotdeviationsButtonPushed(app, [])
                end
            elseif isempty(app.coorOfMasksHexa)==true && isempty(app.coorOfMasksRect)==false && isempty(app.coorOfMasksTri)==true
                SortrectangularButtonPushed(app, [])
                PlotdeviationsButton_3Pushed(app, [])
            elseif isempty(app.coorOfMasksHexa)==true && isempty(app.coorOfMasksRect)==true && isempty(app.coorOfMasksTri)==false
                SorttriangularButtonPushed(app, [])
                PlotdeviationsButton_2Pushed(app, [])
            end
            if isempty(app.directions)==false
                cla(app.UIAxes2_4)
        ...
        % Button pushed function: SorthexagonalButton
        function SorthexagonalButtonPushed(app, event)
                ...
                if isempty(coor)==false
                    ...
                    for i=1:k
                        ...
                        if app.HexagonalButton.Value || app.RectangularButton.Value
                            subplot(1,2,2);
                            hold on
                            plotColorWheel(app,struct('symmetry', 2,'minAngle', minAnglesPerDomain(i)*180/pi, 'maxAngle',maxAnglesPerDomain(i)*180/pi))
                            box off
                        else
                            subplot(1,2,2);
                            hold on
                            plotColorWheel(app,struct('symmetry', 6,'minAngle', minAnglesPerDomain(i)*180/pi, 'maxAngle',maxAnglesPerDomain(i)*180/pi))
                            box off
                        end
                    end
                end
        ...
        % Button pushed function: PlotdeviationsButton_3
        function PlotdeviationsButton_3Pushed(app, event)
        ...
        % Button pushed function: PlotdeviationsButton_2
        function PlotdeviationsButton_2Pushed(app, event)
        ...
        function PlotButton_4Pushed(app, event)
            ...
            if isempty(coor)==false && ~isempty(app.imageP)
                ...
                if app.HexagonalButton.Value || app.RectangularButton.Value
                    figure(50)
                    clf;
                    plotColorWheel(app,struct('symmetry', 2,'minAngle', app.Angle1fromEditField.Value, 'maxAngle',app.Angle2toEditField.Value))
                else
                    figure(50)
                    clf;
                    plotColorWheel(app,struct('symmetry', 6,'minAngle', app.Angle1fromEditField.Value, 'maxAngle',app.Angle2toEditField.Value))
                end
            end
            app.ReadyLamp.Color='green';
            app.ReadyLabel.Text='Ready';
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.ScreeningfactorshexagonalPanel.Position = [30 74 294 83];
            ...
            % Create SorthexagonalButton
            app.SorthexagonalButton = uibutton(app.ScreeningfactorshexagonalPanel, 'push');
            app.SorthexagonalButton.ButtonPushedFcn = createCallbackFcn(app, @SorthexagonalButtonPushed, true);
            app.SorthexagonalButton.Position = [189 22 100 23];
            app.SorthexagonalButton.Text = 'Sort hexagonal';
            ...
            app.ScreeningfactorsrectangularPanel.Position = [25 75 308 82];
            ...
            app.ScreeningfactorstriangularPanel.Position = [23 79 307 82];
            ...
            app.MultiplicativetriangularEditFieldLabel.Position = [9 34 135 22];
            ...
            app.MultiplicativetriangularEditField.Position = [153 33 33 22];
            ...
            app.FractiontriangularEditFieldLabel.Position = [14 5 109 22];
            ...
            app.FractiontriangularEditField.Position = [153 5 34 22];
            ...
            app.SorttriangularButton.Position = [197 22 100 22];
            ...
            % Create PlotdeviationsButton_3
            app.PlotdeviationsButton_3 = uibutton(app.StrainsrectangularTab, 'push');
            app.PlotdeviationsButton_3.ButtonPushedFcn = createCallbackFcn(app, @PlotdeviationsButton_3Pushed, true);
            app.PlotdeviationsButton_3.Position = [83 65 160 23];
            app.PlotdeviationsButton_3.Text = 'Plot deviations';
            ...
            % Create PlotdeviationsButton_2
            app.PlotdeviationsButton_2 = uibutton(app.StrainstriangularTab, 'push');
            app.PlotdeviationsButton_2.ButtonPushedFcn = createCallbackFcn(app, @PlotdeviationsButton_2Pushed, true);
            app.PlotdeviationsButton_2.Position = [87 92 160 23];
            app.PlotdeviationsButton_2.Text = 'Plot deviations';
